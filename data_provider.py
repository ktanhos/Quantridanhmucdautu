...
def _extract_ohlcv(df):
    ...
    df[date_col]=pd.to_datetime(df[date_col]); out=df.set_index(date_col).sort_index()
    keep=[c for c in ['open','high','low','close','volume'] if c in out.columns]
    for c in keep:out[c]=pd.to_numeric(out[c],errors='coerce')
    return out[keep]

def _get_equity_ohlcv(ticker,start_date,end_date):
    market=Market(); start_ts=pd.Timestamp(start_date); end_ts=pd.Timestamp(end_date)
    count=max(int((end_ts-start_ts).days*.72)+50,300)
    raw=market.equity(ticker).ohlcv(end=end_date,interval='1D',count=count)
...
def get_price_data(tickers,start_date,end_date):
    ...
    for ticker in tickers:
        ...
        try:
            if path.exists():
                cached=pd.read_csv(path,parse_dates=['Date'],index_col='Date'); output[ticker]=pd.to_numeric(cached['close'],errors='coerce'); continue
...
def get_market_universe_ohlcv(start_date,end_date,universe='HOSE'):
    ref=Reference(); candidates=[]
    try:
        sectors=normalize_columns(ref.industry.sectors())
        symbol_col=find_col(sectors,['symbol','ticker','stock_code','code'])
        if symbol_col:candidates=sectors[symbol_col].dropna().astype(str).str.upper().unique().tolist()
    except Exception as exc: print(f'Không lấy được universe {universe}: {exc}')
    # Nếu nguồn ngành không trả danh sách, trả rỗng thay vì suy diễn universe.
    if not candidates:return pd.DataFrame()
    prices={}
    for ticker in candidates:
        try:
            df=_get_equity_ohlcv(ticker,start_date,end_date)
            if not df.empty:prices[ticker]=df['close']; pause_api()
        except Exception:continue
    out=pd.DataFrame(prices).sort_index(); out.index=pd.to_datetime(out.index).normalize(); out.index.name='Date'; return out

def get_market_universe_volume(start_date,end_date,universe='HOSE'):
    ref=Reference(); candidates=[]
    try:
        sectors=normalize_columns(ref.industry.sectors()); symbol_col=find_col(sectors,['symbol','ticker','stock_code','code'])
        if symbol_col:candidates=sectors[symbol_col].dropna().astype(str).str.upper().unique().tolist()
    ...
    for ticker in candidates:
        try:
            df=_get_equity_ohlcv(ticker,start_date,end_date)
            if not df.empty and 'volume' in df.columns:volumes[ticker]=df['volume']; pause_api()
        except Exception:continue
    out=pd.DataFrame(volumes).sort_index(); out.index=pd.to_datetime(out.index).normalize(); out.index.name='Date'; return out
