...
def get_price_data(tickers, start_date, end_date) -> pd.DataFrame:
    ...
    for ticker in tickers:
        ...
        try:
            ...
            raw = market.equity(ticker).ohlcv(end=end_date, interval="1D", count=estimated_sessions)
            ohlcv = _extract_ohlcv(raw)
            if ohlcv.empty:
                raise ValueError("API không trả về dữ liệu OHLCV hợp lệ")
            ohlcv = ohlcv[(ohlcv.index >= start_ts) & (ohlcv.index <= end_ts)]
            if len(ohlcv) < 100:
                raise ValueError(f"Chỉ nhận được {len(ohlcv)} phiên")
            ohlcv.rename_axis("Date").to_csv(path)
            output[ticker] = ohlcv["close"].dropna()
            pause_api()
        except Exception as exc:
...
def get_volume_data(tickers, start_date, end_date) -> pd.DataFrame:
    market = Market()
    output = {}
    for ticker in tickers:
        ...
        try:
            if path.exists():
                ...
                if "volume" in cached.columns:
                    output[ticker] = pd.to_numeric(cached["volume"], errors="coerce")
                    continue
            start_ts = pd.Timestamp(start_date)
            end_ts = pd.Timestamp(end_date)
            estimated_sessions = max(int((end_ts - start_ts).days * 0.72) + 50, 300)
            ...
            if "volume" not in ohlcv.columns:
                continue
            ohlcv = ohlcv[(ohlcv.index >= start_ts) & (ohlcv.index <= end_ts)]
            ohlcv[["volume"]].rename_axis("Date").to_csv(path)
            output[ticker] = ohlcv["volume"]
            pause_api()
        except Exception as exc:
            print(f"Lỗi lấy khối lượng {ticker}: {exc}")
    volume = pd.DataFrame(output).sort_index()
...
def get_benchmark_prices(benchmark: str, start_date, end_date) -> pd.Series:
    ...
    while cursor <= end_ts:
        ...
        if close_chunk is None or close_chunk.empty:
            ...
            close_chunk = df["close"].dropna().sort_index()
    ...
    for ticker in tickers:
        ...
        try:
            ...
            if prices is not None and ticker in prices.columns and pd.notna(shares):
                market_cap = float(prices[ticker].dropna().iloc[-1]) * float(shares) * 1000
...
def get_income_data(tickers) -> pd.DataFrame:
    ...
    for ticker in tickers:
        ...
        try:
            ...
            year_cols = sorted([c for c in income.columns if str(c).isdigit() and len(str(c)) == 4], key=lambda x: int(str(x)), reverse=True)
            revenue = profit = np.nan
            revenue_year = profit_year = None
