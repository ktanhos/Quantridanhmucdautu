...
def _trend_score(index):
    ...
    return .4*_score_ratio(r50,.98,1.04)+.4*_score_ratio(r200,.98,1.06)+.2*_score_linear(slope,-.05,.05), {'VNINDEX / MA50':r50,'VNINDEX / MA200':r200,'MA50 21D change':slope}

def _breadth_score(prices):
    if prices is None or prices.empty:return 50.,{'Market Universe stocks':0,'% stocks above MA50':np.nan,'% stocks above MA200':np.nan}
    a50=[];a200=[]
    ...
    return float(np.nanmean([p50,p200])) if np.isfinite(p50) or np.isfinite(p200) else 50.,{'Market Universe stocks':prices.shape[1],'% stocks above MA50':p50,'% stocks above MA200':p200}

def _liquidity_score(v):
    if v is None or v.empty:return 50.,np.nan
    daily=pd.to_numeric(v,errors='coerce').sum(axis=1).dropna()
    if len(daily)<60:return 50.,np.nan
    ratio=daily.tail(20).mean()/daily.tail(60).mean() if daily.tail(60).mean()!=0 else np.nan
...
def calculate_market_regime(index_prices,stock_prices=None,volumes=None):
    ...
    weights={'Trend':.30,'Breadth':.25,'Momentum':.20,'Volatility & Drawdown':.15,'Liquidity':.10}; scores={'Trend':trend_score,'Breadth':breadth_score,'Momentum':momentum_score,'Volatility & Drawdown':risk_score,'Liquidity':liquidity_score}; score=sum(scores[k]*weights[k] for k in weights)
    ...
    indicators={**trend,**breadth,'Return 20D':ret20,'Return 60D':ret60,'Return 120D':ret120,'Volatility 20D':vol20,'Volatility 60D':vol60,'Current Drawdown':drawdown,'VNINDEX Volume 20D / 60D':liquidity_ratio}
