...
def render_portfolio_risk(returns, benchmark_returns):
    ...
    c2.metric('Số vị thế tương đương',f'{effective_positions:.1f}' if np.isfinite(effective_positions) else 'N/A')
    st.caption('HHI là chỉ số đo mức độ tập trung của tỷ trọng. HHI được tính bằng tổng bình phương các tỷ trọng. HHI càng cao thì tiền càng tập trung vào ít mã. Số vị thế tương đương = 1/HHI giúp diễn giải trực quan hơn: HHI 0,250 tương đương mức tập trung của 4 mã được chia đều 25% mỗi mã. HHI này không nói cổ phiếu có rủi ro cao hay thấp và cũng không xét tương quan giữa các mã.')
    st.info(f'Với phân bổ tham chiếu hiện tại, mức tập trung tương đương khoảng {effective_positions:.1f} vị thế. Đây chỉ là mô tả cách chia vốn, không phải kết luận rằng danh mục tốt hay xấu.')
