...
def sortino_ratio(annual_return, series, risk_free_rate=0.0):
    dd = downside_deviation(series, target=risk_free_rate / 252)
    return float((annual_return - risk_free_rate) / dd) if dd > 0 else np.nan
...
def calculate_portfolio_risk(
    ...
    annual_return = float((1 + pr).prod() ** (252 / len(pr)) - 1)
    annual_vol = float(pr.std(ddof=1) * np.sqrt(252))
    sharpe = (annual_return - risk_free_rate) / annual_vol if annual_vol > 0 else np.nan
    sortino = sortino_ratio(annual_return, pr, risk_free_rate)
    mdd = max_drawdown(pr)
    calmar = (annual_return - risk_free_rate) / abs(mdd) if mdd < 0 else np.nan
    ...
    if benchmark_returns is not None:
        ...
        if len(j) >= 20 and j["benchmark"].var(ddof=1) > 0:
            beta = float(
                j["portfolio"].cov(j["benchmark"])
                / j["benchmark"].var(ddof=1)
            )
            active = j["portfolio"] - j["benchmark"]
            tracking_error = float(active.std(ddof=1) * np.sqrt(252))
            portfolio_ann = float((1 + j["portfolio"]).prod() ** (252 / len(j)) - 1)
            benchmark_annual_return = float(
                (1 + j["benchmark"]).prod() ** (252 / len(j)) - 1
            )
            active_return = portfolio_ann - benchmark_annual_return
            information_ratio = (
                float(active_return / tracking_error) if tracking_error > 0 else np.nan
            )
