...
api_key=st.text_input('Mã truy cập Vnstock',type='password')
...
investor_goal=st.radio('Mục tiêu chính',list(goals),horizontal=True); st.caption(goals[investor_goal])
target_return=st.number_input('Lợi nhuận mục tiêu mỗi năm (%)',0.,100.,12.,.5,format='%.1f')
risk_tolerance=st.slider('Khẩu vị rủi ro',0,100,50,5); risk_capacity=st.slider('Khả năng chịu rủi ro',0,100,50,5)
...
liquidity_need=st.selectbox('Nhu cầu sử dụng tiền',['Cao','Trung bình','Thấp'])
...
allow_short=st.checkbox('Cho phép bán khống'); allow_leverage=st.checkbox('Cho phép sử dụng đòn bẩy')
...
if st.button('LƯU HỒ SƠ ĐẦU TƯ',type='primary',use_container_width=True,disabled=bool(errors)):
...
with col2:
    end_date=st.date_input('Ngày kết thúc',value=pd.Timestamp.today().date()); benchmark_data=st.text_input('Benchmark',value=benchmark or DEFAULT_BENCHMARK).strip().upper()
