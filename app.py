...
allow_short=False;st.checkbox('Cho phép bán khống',value=False,disabled=True,help='Phiên bản hiện tại không hỗ trợ bán khống.');allow_leverage=st.checkbox('Cho phép vay Margin',value=False,help='Chỉ dùng để định hướng mức độ chấp nhận vốn vay.');margin_rate=st.number_input('Lãi suất vay Margin (%/năm)',9.,15.,12.,.25,format='%.2f',disabled=not allow_leverage);defensive_asset=st.radio('Tài sản phòng thủ',['Tiền mặt','Tiền gửi ngắn hạn'],horizontal=True);benchmark=st.text_input('Benchmark',value=DEFAULT_BENCHMARK).strip().upper()
