...
<div class="hero-subtitle">Ứng dụng giúp bạn trả lời từng câu hỏi trước khi đầu tư: nên dành bao nhiêu tiền cho cổ phiếu, thị trường hiện thuận lợi hay cần thận trọng, nên chọn những cổ phiếu nào, chia tiền ra sao, có nên dùng tiền vay và danh mục đang hoạt động thế nào.</div>
<span class="hero-chip">Mục tiêu</span><span class="hero-chip">Phân bổ tiền</span><span class="hero-chip">Thị trường</span><span class="hero-chip">Chọn cổ phiếu</span><span class="hero-chip">Tỷ trọng</span><span class="hero-chip">Tiền vay</span>
</div>
""",unsafe_allow_html=True)

with st.sidebar:
    st.markdown("### Các bước quản trị danh mục")
    steps=[
        ("1","Mục tiêu của bạn",st.session_state.get("policy") is not None),
        ("2","Phân bổ tiền",st.session_state.get("regime_result") is not None),
        ("3","Đánh giá thị trường","market_data" in st.session_state),
        ("4","Chọn cổ phiếu","market_data" in st.session_state),
        ("5","Chia tỷ trọng","optimization_result" in st.session_state),
        ("6","Kiểm soát tiền vay","complete_portfolio_result" in st.session_state),
        ("7","Theo dõi danh mục","portfolio_performance" in st.session_state),
    ]
    for number,label,done in steps:
        mark="Đã sẵn sàng" if done else "Chưa thực hiện";color="#62d39b" if done else "#8f96a3"
    ...
    st.divider();st.caption("Không đặt lệnh. Ứng dụng hỗ trợ ra quyết định và kiểm soát rủi ro.")

with st.expander("Chi phí và giả định tham khảo",expanded=False):
    c1,c2,c3,c4=st.columns(4)
    c1.metric("Phí giao dịch mua/bán","0,10%");c2.metric("Thuế khi bán","0,10%");c3.metric("Phí lưu ký","0,27 đồng/cổ phiếu/tháng");c4.metric("Lãi suất vay","12%/năm")
    st.caption("Các thông số chỉ mang tính tham khảo và cần cập nhật theo điều kiện thực tế.")

st.header("Bước 1. Mục tiêu và mức chấp nhận rủi ro")
st.markdown('<div class="section-note">Trước khi chọn cổ phiếu, hãy xác định bạn muốn đạt gì, có bao nhiêu vốn và chấp nhận danh mục giảm bao nhiêu khi thị trường xấu.</div>',unsafe_allow_html=True)
...
risk_profile=st.radio("Mức chấp nhận biến động",list(risk_options),horizontal=True,key="risk_profile");risk_score=risk_options[risk_profile];st.info(risk_profile_description(risk_score))
...
with st.expander("Tiền dự phòng và tiền vay",expanded=True):
    c1,c2=st.columns(2)
    with c1:
        defensive_asset=st.radio("Nơi giữ phần tiền chưa đầu tư",["Tiền mặt","Tiền gửi ngắn hạn"],horizontal=True,key="defensive_asset")
        emergency_cash_percent=st.number_input("Tiền mặt dự phòng (%)",0.,100.,0.,1.,format="%.1f",key="emergency_cash")
    with c2:
        allow_leverage=st.checkbox("Cho phép sử dụng tiền vay",value=False,key="allow_leverage",help="Cho phép đầu tư vượt vốn tự có trong giới hạn đã đặt.")
        margin_rate=st.number_input("Lãi suất vay (%/năm)",9.,20.,12.,.25,format="%.2f",disabled=not allow_leverage,key="margin_rate")
        max_leverage=st.number_input("Mức đầu tư tối đa so với vốn tự có (lần)",1.,3.,2.,.1,format="%.1f",disabled=not allow_leverage,key="max_leverage")
benchmark=st.text_input("Chỉ số dùng để so sánh",value=DEFAULT_BENCHMARK,key="benchmark_profile").strip().upper()
risk_free_rate=st.number_input("Lãi suất tham chiếu (%/năm)",0.,15.,4.,.25,format="%.2f",key="risk_free_rate")
policy=InvestmentPolicy(investor_goal=investor_goal,target_return=target_return/100,risk_tolerance=risk_score,risk_capacity=risk_score,investment_horizon_years=5,liquidity_need="Không đặt trong phiên bản hiện tại",benchmark=benchmark or DEFAULT_BENCHMARK,max_single_stock_weight=max_single_stock_weight/100,max_sector_weight=max_sector_weight/100,allow_short=False,allow_leverage=allow_leverage,defensive_asset=defensive_asset,emergency_cash_percent=emergency_cash_percent/100,risk_free_rate=risk_free_rate/100)
policy_dict=policy.to_dict();policy_dict.update({"margin_rate":margin_rate/100,"max_leverage":max_leverage})
errors=validate_policy(policy)
if errors:
    for e in errors:st.warning(e)
else:st.success(f"Thiết lập hợp lệ. Mục tiêu {target_return:.1f}% mỗi năm. Mức chấp nhận biến động: {risk_label(risk_score)}.")
if st.button("LƯU THIẾT LẬP",type="primary",use_container_width=True,key="save_policy",disabled=bool(errors)):
    st.session_state["policy"]=policy_dict;st.session_state["investment_capital"]=investment_capital;st.session_state["saved_margin_rate"]=margin_rate/100;st.session_state["saved_max_leverage"]=max_leverage
    ...
    st.success("Đã lưu thiết lập. Hệ thống sẽ tiếp tục theo từng bước quản trị danh mục.")

st.header("Bước 2. Phân bổ tiền theo tình trạng thị trường")
st.markdown('<div class="section-note">Trước khi chọn từng cổ phiếu, cần quyết định tổng cộng bao nhiêu tiền nên dành cho cổ phiếu và bao nhiêu nên giữ ở phần phòng thủ.</div>',unsafe_allow_html=True)
...
with col2:benchmark_data=st.text_input("Chỉ số dùng để đánh giá thị trường",value=benchmark or DEFAULT_BENCHMARK,key="benchmark_data").strip().upper()
col1,col2=st.columns(2)
...
if st.button("LẤY DỮ LIỆU VÀ XÁC ĐỊNH MỨC ĐẦU TƯ CỔ PHIẾU",type="secondary",use_container_width=True,key="load_data"):
    if st.session_state.get("policy") is None:st.warning("Hãy lưu Thiết lập trước.");st.stop()
    ...
    try:
        with st.spinner("Đang lấy dữ liệu và đánh giá tình trạng thị trường..."):
            data=load_market_dataset(tickers,pd.Timestamp(start_date),pd.Timestamp(end_date),benchmark_data or DEFAULT_BENCHMARK);st.session_state["market_data"]=data
...
if "market_data" in st.session_state:
    ...
    st.header("Bước 3. Đánh giá thị trường và rủi ro chung")
    ...
    st.subheader("Chất lượng dữ liệu");st.dataframe(quality,use_container_width=True,hide_index=True)
    ...
    if regime_result is not None:
        st.session_state["asset_allocation_result"]={"regime":getattr(regime_result,"regime","Trung tính"),"score":float(getattr(regime_result,"score",50)),"equity_min":float(getattr(regime_result,"equity_min",0.5)),"equity_max":float(getattr(regime_result,"equity_max",0.7)),"defensive_min":1.0-float(getattr(regime_result,"equity_max",0.7)),"defensive_max":1.0-float(getattr(regime_result,"equity_min",0.5))}
        a=st.session_state["asset_allocation_result"]
        st.subheader("Mức tiền nên dành cho cổ phiếu")
        c1,c2,c3,c4=st.columns(4);c1.metric("Tình trạng thị trường",a["regime"]);c2.metric("Điểm đánh giá",f'{a["score"]:.1f}/100');c3.metric("Cổ phiếu tối thiểu",f'{a["equity_min"]:.0%}');c4.metric("Cổ phiếu tối đa",f'{a["equity_max"]:.0%}')
        st.caption(f'Phần tiền còn lại được dành cho {policy_dict.get("defensive_asset","tài sản phòng thủ")}, trong khoảng {a["defensive_min"]:.0%} đến {a["defensive_max"]:.0%}.')

    st.header("Bước 4. Chọn cổ phiếu phù hợp")
    st.markdown('<div class="section-note">Một cổ phiếu có triển vọng tốt chưa chắc phù hợp khi đặt cạnh các cổ phiếu khác. Hệ thống xem xét lợi suất kỳ vọng, mức biến động và mức độ các cổ phiếu biến động giống nhau.</div>',unsafe_allow_html=True)
    rf=float((st.session_state.get("policy") or {}).get("risk_free_rate",0.04));render_portfolio_risk(data["returns"],data["benchmark_returns"],risk_free_rate=rf)

    st.header("Bước 5. Chia tỷ trọng cho từng cổ phiếu")
    st.markdown('<div class="section-note">Sau khi biết tổng tiền dành cho cổ phiếu, hệ thống tìm cách chia tiền giữa các mã để tránh tập trung quá mức vào một cổ phiếu hoặc một nguồn rủi ro.</div>',unsafe_allow_html=True)
    ...
    if "target_equity" in st.session_state:
        st.header("Bước 7. Theo dõi và đánh giá danh mục")
        st.markdown('<div class="section-note">Không chỉ nhìn lợi nhuận. Hãy xem danh mục biến động bao nhiêu, giảm mạnh nhất bao nhiêu và đang tốt hơn hay kém hơn mục tiêu của bạn.</div>',unsafe_allow_html=True)
    ...
    if "portfolio_performance" in st.session_state:
        target_equity_total=float(pd.Series(st.session_state.get("target_equity"),dtype=float).sum());st.divider();render_portfolio_summary(st.session_state.get("portfolio_performance"),st.session_state.get("regime_result"),None,target_equity_total)
