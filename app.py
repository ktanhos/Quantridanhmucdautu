...
st.caption('Công cụ định hướng quản trị danh mục cổ phiếu Việt Nam theo dữ liệu thị trường và hồ sơ nhà đầu tư.')
...
if st.button('LƯU HỒ SƠ ĐẦU TƯ',type='primary',use_container_width=True,disabled=bool(errors),key='save_policy'):st.session_state['policy']=policy.to_dict();st.session_state['saved_margin_rate']=margin_rate/100;st.success('Đã lưu hồ sơ.')
...
if 'market_data' in st.session_state:
    ...
    c1,c2,c3,c4=st.columns(4);c1.metric('Số mã',len(data['prices'].columns));c2.metric('Số phiên',len(data['prices']));c3.metric('Ngày bắt đầu',pd.Timestamp(data['start_date']).strftime('%d/%m/%Y'));c4.metric('Ngày kết thúc',pd.Timestamp(data['end_date']).strftime('%d/%m/%Y'))
    ...
    quality=quality.rename(columns={'Mã':'Mã','Giá hiện tại':'Giá hiện tại (VNĐ)','Tăng/Giảm 1M':'Tăng/Giảm 1M','Tăng/Giảm 6M':'Tăng/Giảm 6M','Tăng/Giảm 12M':'Tăng/Giảm 12M','Biến động Annualized':'Volatility Annualized','Khối lượng TB':'Average Volume','Số phiên có dữ liệu':'Số phiên dữ liệu','Số phiên thiếu':'Số phiên thiếu','Độ phủ dữ liệu':'Data Coverage'})
    st.subheader('Thống kê thị trường và chất lượng dữ liệu')
    st.dataframe(quality,use_container_width=True,hide_index=True)
    st.caption('1M tương ứng khoảng 21 phiên giao dịch, 6M khoảng 126 phiên và 12M khoảng 252 phiên. Volatility Annualized là độ biến động được quy đổi về cơ sở một năm từ lợi suất ngày. Average Volume là khối lượng giao dịch bình quân mỗi phiên. Data Coverage là tỷ lệ phiên có dữ liệu hợp lệ.')
