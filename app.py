...
st.markdown("""<style>.block-container {max-width: 1250px; padding-top: 2rem;}.help-text {color:#6b7280; font-size:0.92rem;}</style>""", unsafe_allow_html=True)
...
with st.container(border=True):
    ...
    investor_goal = st.radio("Mục tiêu chính", list(goal_options), horizontal=True)
    st.caption(goal_options[investor_goal])
    target_return = st.number_input("Lợi nhuận mục tiêu mỗi năm (%)", 0.0, 100.0, 12.0, 0.5, format="%.1f", help="Mức lợi nhuận mong muốn bình quân mỗi năm. Đây là mục tiêu, không phải cam kết.")
with st.container(border=True):
    st.subheader("2.2. Bạn chấp nhận biến động đến mức nào?")
    risk_tolerance = st.slider("Khẩu vị rủi ro", 0, 100, 50, 5)
    st.caption(f"Khẩu vị hiện tại: **{risk_label(risk_tolerance)}**.")
    risk_capacity = st.slider("Khả năng chịu rủi ro", 0, 100, 50, 5)
    st.caption(f"Khả năng hiện tại: **{risk_label(risk_capacity)}**.")
with st.container(border=True):
    ...
    investment_horizon_years = st.select_slider("Thời hạn đầu tư", options=list(horizon_labels), value=7, format_func=lambda x: horizon_labels[x])
    liquidity_options = {"Cao": "Có thể cần tiền trong ngắn hạn.", "Trung bình": "Có thể chấp nhận một phần vốn biến động trong thời gian dài hơn.", "Thấp": "Ít nhu cầu sử dụng vốn trong ngắn hạn."}
    liquidity_need = st.selectbox("Nhu cầu sử dụng tiền", list(liquidity_options), index=1)
    st.caption(liquidity_options[liquidity_need])
with st.container(border=True):
    st.subheader("2.4. Các giới hạn bạn muốn đặt cho danh mục")
    c1, c2, c3 = st.columns(3)
    with c1: max_single_stock_weight = st.number_input("Tối đa một cổ phiếu (%)", 1.0, 100.0, 10.0, 1.0, format="%.1f")
    with c2: max_sector_weight = st.number_input("Tối đa một ngành (%)", 1.0, 100.0, 25.0, 1.0, format="%.1f")
    with c3: emergency_cash_percent = st.number_input("Tiền dự phòng tối thiểu (%)", 0.0, 100.0, 10.0, 1.0, format="%.1f")
    c4, c5 = st.columns(2)
    with c4: allow_short = st.checkbox("Cho phép bán khống", value=False)
    with c5: allow_leverage = st.checkbox("Cho phép sử dụng đòn bẩy", value=False)
with st.container(border=True):
    st.subheader("2.5. Nếu thị trường trở nên rủi ro, bạn muốn hệ thống chuyển tiền sang đâu?")
    defensive_asset = st.radio("Tài sản phòng thủ", ["Tiền mặt", "Tiền gửi ngắn hạn"], horizontal=True)
with st.container(border=True):
    st.subheader("2.6. Thị trường dùng để so sánh")
    benchmark = st.text_input("Chỉ số tham chiếu", value=DEFAULT_BENCHMARK).strip().upper()
...
if policy_errors:
    for error in policy_errors: st.warning(error)
...
st.caption("Dữ liệu ngày gồm giá đóng cửa và khối lượng giao dịch của từng cổ phiếu, cùng benchmark. Khối lượng được dùng để đánh giá thanh khoản và xác nhận xu hướng.")
col1, col2 = st.columns(2)
with col1:
    tickers_text = st.text_input("Các mã cổ phiếu muốn theo dõi", value=", ".join(DEFAULT_TICKERS))
...
if run:
    ...
    try:
        with st.spinner("Đang lấy dữ liệu giá, khối lượng, benchmark và dữ liệu doanh nghiệp..."):
...
if "market_data" in st.session_state:
    ...
    c1.metric("Số mã", len(data["prices"].columns)); c2.metric("Số phiên", len(data["prices"])); c3.metric("Ngày bắt đầu", pd.Timestamp(data["start_date"]).strftime("%d/%m/%Y")); c4.metric("Ngày kết thúc", pd.Timestamp(data["end_date"]).strftime("%d/%m/%Y"))
    with st.expander("Xem chất lượng dữ liệu", expanded=True): st.dataframe(data["data_quality"], use_container_width=True, hide_index=True)
    with st.expander("Xem dữ liệu giá", expanded=False): st.dataframe(data["prices"].tail(20), use_container_width=True)
    with st.expander("Xem khối lượng giao dịch", expanded=False): st.dataframe(data["volumes"].tail(20), use_container_width=True)
    with st.expander("Xem lợi suất", expanded=False): st.dataframe(data["returns"].tail(20), use_container_width=True)
    ...
    with st.expander("Xem thông tin doanh nghiệp", expanded=False): st.dataframe(data["company_table"], use_container_width=True, hide_index=True)
    st.divider()
    render_market_regime(data["benchmark_prices"], data["prices"], data["volumes"])
