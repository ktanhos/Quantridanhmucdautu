...
def build_complete_portfolio(orp_weights,expected_returns,covariance,risk_free_rate,risk_tolerance,
                             allow_leverage=None,margin_rate=None,max_leverage=1.50,current_weights=None,**kwargs):
    ...
    Chi phí vốn vay được tính theo lãi suất Margin và được trừ khỏi lợi suất kỳ vọng.
    """
    ...
    if allow_leverage and margin>rf and variance_orp>1e-12:
        y_levered_raw=float((er_orp-margin)/(A*variance_orp))
    else:
        y_levered_raw=y_unlevered_raw
    if allow_leverage:
        y=float(np.clip(y_levered_raw,0.0,float(max_leverage)))
    else:
        y=float(np.clip(y_unlevered_raw,0.0,1.0))
    complete_equity=w_orp*y
    borrowed=max(0.0,y-1.0); defensive=max(0.0,1.0-y); borrowing_cost=borrowed*margin
    ...
    return {'orp_weights':w_orp,'complete_equity_weights':complete_equity,'defensive_weight':defensive,'borrowed_weight':borrowed,
            'expected_return_orp':er_orp,'volatility_orp':sigma_orp,'variance_orp':variance_orp,'risk_free_rate':rf,'margin_rate':margin,
            'risk_aversion':A,'excess_return':excess,'y_raw_no_borrow':y_unlevered_raw,'y_raw_with_margin':y_levered_raw,'y':y,
            'complete_expected_return':complete_return,'complete_volatility':complete_vol,'complete_sharpe':complete_sharpe,
            'borrowing_cost':borrowing_cost,'allow_leverage':bool(allow_leverage),'max_leverage':float(max_leverage),'comparison':comparison}
